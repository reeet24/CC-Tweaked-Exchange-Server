...
local latestClientVersion = "v0.9"
...
dispatcher.on("request_challenge", function(sender, packet)
    ...
    print("Challenge created for", sender, ":", challenge)
...
dispatcher.on("get_user_inventory", function(sender, packet)
    local sessionKey = packet.data.sessionKey
    local publicKey = packet.data.publicKey

    local userId = keyring.resolvePublicKey(publicKey)
    if not userId then
        net.respond(sender, packet.id, {
            success = false,
            error = "Invalid public key"
    ...
    local valid, sessionErr = auth.verifySession(sessionKey, userId)
    if not valid then
        net.respond(sender, packet.id, {
            success = false,
            error = sessionErr or "Session invalid"
        })
        return
    end

    local userData = users.get(userId)
    if not userData or not userData.InventoryServerId then
        net.respond(sender, packet.id, {
            success = false,
            error = "InventoryServerId not set for user"
        })
        return
    end

    local inventoryID = userData.InventoryServerId
    if not peripheral.isPresent(inventoryID) then
        net.respond(sender, packet.id, {
            success = false,
            error = "Inventory peripheral '" .. inventoryID .. "' not found"
        })
        return
    end

    local ok, invList = pcall(peripheral.call, inventoryID, "list")
    if not ok then
        net.respond(sender, packet.id, {
            success = false,
            error = "Inventory list failed: " .. tostring(invList)
        })
        return
    end

    -- Collapse entries by item name
    local collapsed = {}
    for slot, item in pairs(invList) do
        local key = item.name
        if collapsed[key] then
            collapsed[key].count = collapsed[key].count + item.count
        else
            collapsed[key] = {
                name = item.name,
                count = item.count
...
dispatcher.on("get_inventory_slot_detail", function(sender, packet)
    local sessionKey = packet.data.sessionKey
    local publicKey = packet.data.publicKey
    local slot = tonumber(packet.data.slot)
    ...
    local ok, result = pcall(peripheral.call, inventoryID, "getItemDetail", slot)
    if not ok then
        net.respond(sender, packet.id, {
            success = false,
            error = "Item detail failed: " .. tostring(result)
        })
    else
        net.respond(sender, packet.id, {
            success = true,
...
scheduler:addJob("run_main_loop", 1, function ()
    scheduler:removeJob("run_main_loop")
